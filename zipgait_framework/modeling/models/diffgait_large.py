...
class DiffGait_Large(BaseModel):
    ...
    def forward(self, inputs):
        ...
        ### denoising
        outs = self.diffgait_denoising_branch(self.DFModel, maps)
        outs = rearrange(outs, 'n c s h w -> (n s) c h w')
        sils = rearrange(sils, 'n c s h w -> (n s) c h w')
        maps = rearrange(maps, 'n c s h w -> (n s) c h w')
        save_dir = f"/home/gsx/mfx/diffgait_gait3d/diffgait_outs_ts_2/{labs[0]}/{v[0]}/{s[0]}"
